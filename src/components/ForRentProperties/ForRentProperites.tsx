...
const ForRentProperties = () => {
  ...
  const itemsToShow = forRentProperties.slice(startIndex, endIndex);
  return (
    <>
      <Separator />
      <section className="bg-paper -mt-[1px]">
        <div className="relative z-10 xl:custom-container pt-10 md:pt-0 pb-10">
          <h1 className="text-center text-primary-dark font-medium">
            Properties
          </h1>
          <h2 className="text-center text-3xl">For Rent</h2>
          ...
          >
            ...
            >
              {itemsToShow.map((property) => (
                ...
                >
                  <ForRentPropertyItem data={property} />
                </div>
              ))}
            </div>
            {loading && (
